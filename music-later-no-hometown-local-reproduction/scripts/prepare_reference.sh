...
EXPECTED_SHA256="176c81b30cbd68de6cd7c900d1513160f9a6a591eb7e9bed65d848f273c48b0e"
...
ACTUAL_SHA256="$(shasum -a 256 "$SRC" | awk '{print $1}')"
if [[ "$ACTUAL_SHA256" != "$EXPECTED_SHA256" ]]; then
  echo 'ERROR: reference SHA-256 does not match the approved source.' >&2
...
SHA_FILE="$PRIVATE_ROOT/后来没有故乡.sha256"

cp -p "$SRC" "$LOCAL_M4A"
printf '%s  %s\n' "$EXPECTED_SHA256" "$(basename "$LOCAL_M4A")" > "$SHA_FILE"
